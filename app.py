...
if uploaded_file:
    try:
        ...
        stats = pd.DataFrame({
            ...
            "Moda": df[colunas_selecionadas].apply(
                lambda x: ", ".join(map(str, x.mode().tolist())) if not x.mode().empty else "N/A"
            ),
        ...
        st.info("💡 Dica: Abra o arquivo baixado no navegador e pressione Ctrl+P → 'Salvar como PDF' para ter o PDF perfeito!")
